...
export class Sidebar implements Destroyable {
  ...
  constructor(
    ...
    this.toolbar = new ToolbarController(toolbarContainer, {
      ...
      setHighlightsVisible: visible => this.setHighlightsVisible(visible),
      setSilentMode: mode => this.setIsSilent(mode),
      toggleChatting: value => this.turnOnChat(value),
      toggleRecording: status => {
        this._sidebarRPC.call('toggleRecording', status);
      },
    });
  ...
  _setupSidebarEvents() {
    ...
    this._sidebarRPC.on('openImageViewer', (data) => {
      this._emitter.publish('openImageViewer', data);
    });
    this._emitter.subscribe('closeImageViewer', (data: {id: string}) => {
      this._sidebarRPC.call('closeImageViewer', data);
      this.show();
    });
