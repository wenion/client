...
import type { VideoDraft } from '../../store/modules/video-drafts';
import MarkdownEditor from '../MarkdownEditor';
import TagEditor from '../TagEditor';
import AnnotationLicense from './VideoAnnotationLicense';
import VideoAnnotationPublishControl from './VideoAnnotationPublishControl';
...
function VideoAnnotationEditor({
  ...
  const onEditTags = useCallback(
    (tags: string[]) => {
      store.createVideoDraft(draft.annotation, { ...draft, tags });
    },
    [draft, store]
  );
  ...
  const onEditText = useCallback(
    (text: string) => {
      store.createVideoDraft(draft.annotation, { ...draft, text });
    },
    [draft, store]
  );
  ...
  const onSave = async () => {
    ...
    try {
      await videoAnnotationsService.save(annotation);
  ...
  const onCancel = useCallback(() => {
    store.removeVideoDraft(annotation);
    if (!isSaved(annotation)) {
      store.removeVideoAnnotations([annotation]);
    }
  }, [annotation, store]);
...
export default withServices(VideoAnnotationEditor, [
  'videoAnnotationsService',
  'settings',
  'tags',
  'toastMessenger',
